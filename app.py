...
<title>URL Video — descarga contenido autorizado</title>
<style>body{margin:0;background:#0b1020;color:#edf2ff;font:16px system-ui,sans-serif}.box{max-width:720px;margin:10vh auto;padding:38px;background:#121a30;border:1px solid #28365d;border-radius:20px}h1{margin:0 0 8px}p{color:#b9c4db;line-height:1.55}form{display:grid;gap:16px;margin-top:28px}input,select,button{font:inherit;border-radius:10px;padding:14px;border:1px solid #34436c;background:#0b1020;color:#fff}button{background:#765cff;border:0;font-weight:700;cursor:pointer}.note{font-size:13px;color:#94a3bd}label{display:grid;gap:7px}a{color:#a99bff}</style></head>
<body><main class="box"><h1>URL Video</h1><p>Pega un enlace público y descarga una copia en MP4 o MP3. El archivo se genera temporalmente y se elimina del servidor después de la descarga.</p>
<form method="post" action="/download"><label>URL pública<input name="url" type="url" required placeholder="https://ejemplo.com/video"></label><label>Formato<select name="format"><option value="mp4">Vídeo MP4</option><option value="mp3">Audio MP3</option></select></label><label class="note"><input type="checkbox" name="rights" value="yes" required> Confirmo que el contenido es mío, público o tengo permiso para descargarlo.</label><button type="submit">Preparar descarga</button></form><p class="note">No se admiten contenidos privados, de pago ni protegidos por DRM. Software de Nokhoi Connect.</p></main></body></html>"""
